...
function Showcase({ onOpen }) {
  const hero = [
    ...
    { img: "images/soleil-d-or-noir.jpg", name: "Soleil d'Or Noir", tag: "Contraste or & noir · marqueterie", size: "wide", pos: "center 30%", zoom: 1.1 },
  ];
  return (
        ...
        <div className="showcase-grid">
          {hero.map((h, i) => (
            <div key={i} className={"showcase-item reveal sc-" + h.size} onClick={() => onOpen({img: h.img, name: h.name})}>
              <div className="showcase-frame"><img src={h.img} alt={h.name} style={{...(h.pos ? {objectPosition: h.pos} : {}), ...(h.zoom ? {transform: `scale(${h.zoom})`} : {})}} /></div>
